package com.nandaadisaputra.master.talkingjerry_master
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn_tts.onClick {
            if (edt_teks.text.toString().isBlank()) {
                toast("Helooowww, teksnya kosong bro!!")
            } else {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    textToSpeech!!.speak(edt_teks.text.toString(), TextToSpeech.QUEUE_FLUSH, null, null)
                } else {
                    textToSpeech!!.speak(edt_teks.text.toString(), TextToSpeech.QUEUE_FLUSH, null)
                }
